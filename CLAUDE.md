...
## Secrets
Three mechanisms, each scoped to a different point in the config lifecycle — reach for the one that matches, don't blend them:
...
- **`op read` at home-manager activation** — for MCP server secrets (env vars and `Authorization` headers), e.g. `grafana` on Jeffs-M3Pro (`hosts/home/jeff/Jeffs-M3Pro.nix`) and `leanix` on work-laptop (`hosts/home/jeff/work-laptop.nix`). Resolved once per `nh home switch` and baked into `~/.claude.json`; a locked vault at switch time fails the switch rather than silently registering an empty token.

    ...
    in
    {
      ...
      config = lib.mkIf cfg.enable {
        ...
        systemd.services.garage = {
          # mnt-ssdtank-garage.mount is only *synthesized* from mountinfo after `zfs mount -a`
          # runs, so it does not exist when systemd computes the boot transaction. A slow pool
          # import (e.g. `tank`) delays `zfs-mount.service`, leaving the mount in `not-found`
          # state and causing garage's WantedBy job to be silently pruned (observed 2026-07-05:
          # garage never started after a reboot, with zero journal entries and no failed unit).
          # Depend on the real zfs-mount.service (which guarantees the dataset is mounted)
          # instead.
          requires = [
            "zfs-mount.service"
